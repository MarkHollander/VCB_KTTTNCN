...
using Qlud.KTTTNCN.Authentication.TwoFactor;
using Qlud.KTTTNCN.Chat;
using Qlud.KTTTNCN.Configuration;
using Qlud.KTTTNCN.EntityFrameworkCore;
using Qlud.KTTTNCN.Startup;
using Qlud.KTTTNCN.Web.Authentication.JwtBearer;
using Qlud.KTTTNCN.Web.Authentication.TwoFactor;
using Qlud.KTTTNCN.Web.Chat.SignalR;
using Qlud.KTTTNCN.Web.Common;
using Qlud.KTTTNCN.Web.Configuration;
using Qlud.KTTTNCN.Web.DashboardCustomization;

namespace Qlud.KTTTNCN.Web
{
    [DependsOn(
        typeof(KTTTNCNApplicationModule),
        typeof(KTTTNCNEntityFrameworkCoreModule),
        typeof(AbpAspNetZeroCoreWebModule),
        typeof(AbpAspNetCoreSignalRModule),
        typeof(KTTTNCNGraphQLModule),
        typeof(AbpRedisCacheModule), //AbpRedisCacheModule dependency (and Abp.RedisCache nuget package) can be removed if not using Redis cache
        typeof(AbpHangfireAspNetCoreModule) //AbpHangfireModule dependency (and Abp.Hangfire.AspNetCore nuget package) can be removed if not using Hangfire
    )]
    public class KTTTNCNWebCoreModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public KTTTNCNWebCoreModule(IWebHostEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(KTTTNCNApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(KTTTNCNWebCoreModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            IocManager.Resolve<ApplicationPartManager>()
                .AddApplicationPartsIfNotAddedBefore(typeof(KTTTNCNWebCoreModule).Assembly);
        }
