...
using Qlud.KTTTNCN.Configure;
using Qlud.KTTTNCN.EntityFrameworkCore;
using Qlud.KTTTNCN.Identity;
using Qlud.KTTTNCN.Schemas;
using Qlud.KTTTNCN.Web.Chat.SignalR;
using Qlud.KTTTNCN.Web.Common;
using Qlud.KTTTNCN.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using Qlud.KTTTNCN.Web.IdentityServer;
using Qlud.KTTTNCN.Web.Swagger;
using Stripe;
...
using Qlud.KTTTNCN.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using Qlud.KTTTNCN.Web.Extensions;
using Qlud.KTTTNCN.Web.MultiTenancy;
using SecurityStampValidatorCallback = Qlud.KTTTNCN.Identity.SecurityStampValidatorCallback;
...
namespace Qlud.KTTTNCN.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<KTTTNCNWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                app.UseKTTTNCNForwardedHeaders();
            }
            else
            {
            ...
            if (KTTTNCNConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "KTTTNCN API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("Qlud.KTTTNCN.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "KTTTNCN API", Version = "v1"});
                ...
                var applicationXml = $"Qlud.KTTTNCN.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"Qlud.KTTTNCN.Web.Core.xml";
